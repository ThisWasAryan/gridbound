  ...
  window.Game = {
    ...
    debug: {
      ...
      validateState() {
        const errors = [];
        if (
          typeof gameState.economy.credits !== "number" ||
          isNaN(gameState.economy.credits)
        )
          errors.push("Invalid credits.");
        if (gameState.economy.credits < 0) errors.push("Negative credits.");
        if (!gameState.cars[gameState.session.currentCarId]?.owned)
          errors.push(
            `Current car ${gameState.session.currentCarId} is not owned.`,
          );
        if (!gameState.tracks[gameState.session.currentTrackId]?.owned)
          errors.push(
            `Current track ${gameState.session.currentTrackId} is not owned.`,
          );
